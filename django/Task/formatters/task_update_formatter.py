...
class TaskUpdateFormatter:
    ...
    def format(self):
        return ServiceResponse('Task data formatted for DB').data(self.td)

    def isTaskDone(self):
        return self.td['status'] == '1' if 'status' in self.task_data else False
    ...
    def _generate(self):
        ...
        if 'add_to_my_day' in self.task_data:
            td['my_day_date'] = date.today() if self.task_data['add_to_my_day'] else None
